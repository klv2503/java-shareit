...
public class BookingServiceImpl implements BookingService {
    ...
    @Override
    public List<BookingOutputDto> getAllUsersBookings(Long id, String state) {
        String upperState = state.toUpperCase();
        User booker = userService.getUser(id);
        List<Booking> bookingList = switch (upperState) {
            case "ALL" -> repository.findAllByBooker(booker);
            case "WAITING", "REJECTED" -> repository.findAllByBookerAndStatus(booker, BookStatus.valueOf(upperState));
            case "PAST" -> repository.findAllByBookerAndEndBefore(booker, LocalDateTime.now());
            case "FUTURE" -> repository.findAllByBookerAndStartAfter(booker, LocalDateTime.now());
            case "CURRENT" -> repository.findCurrentUsersBookings(booker, LocalDateTime.now());
